...
for i in range(steps):
    ...
    # Access Camera image and save image
    current_path=os.getcwd()
    path_image = current_path + '/main_Camera.png'
    sensors[2].save(path_image, compression=0)

    # Access the image by calling it with openCV functions
    image_read = cv2.imread(path_image,1)
    image_read  = cv2.resize(image_read , (300, 300))
    cv2.imshow('Camera Stream F1TENTH car', image_read)
    cv2.waitKey(1)
